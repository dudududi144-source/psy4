...
// POST /api/telemetry/radio  → log radio analysis to LOCAL DB
    ...
    if (process.env.TURSO_DATABASE_URL && process.env.TURSO_AUTH_TOKEN) {
      try {
        ...
        if (isTursoConfigured()) {
          await ensureSchema();
          await tursoBatch([{
            sql: `INSERT INTO radio_telemetry (stream_name, bpm, warmth, brightness, loudness, smoothness, style, in_breakdown, measured_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            args: [body.streamName, body.bpm ?? 0, body.warmth ?? 0, body.brightness ?? 0, body.loudness ?? 0, body.smoothness ?? 0, body.style ?? 'UNKNOWN', body.inBreakdown ? 1 : 0, Date.now()],
          }]);
        }
      } catch (err) {
